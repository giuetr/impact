...
const dataGenerator = (index) => {
    const referenceValue = _.random(70, 200);
    const halfedRefVal = referenceValue / 2;

    return {
        key: index,
        month: moment(CHART_START_DATE).add(index, 'months').format('MMM YY'),
        "Institutional": referenceValue,
        "Commercial": _.random(100, 160),
        "Retail": referenceValue - _.random(halfedRefVal, halfedRefVal * 1.1),
    };
}
...
export const AudienceMetricsChart = ({height, className}) => (
    ...
    >
        <ComposedChart data={data}
          ...
          <Tooltip />
          <Legend />
          <Bar dataKey='Institutional' barSize={5} fill={ colors['400'] } />
          <Area dataKey='Commercial' fill={ colors['purple-02'] } stroke={ colors['purple'] } activeDot={ null } />
          <Area dataKey='Retail' fill={ colors['primary-04'] } stroke={ colors['primary'] } activeDot={{r: 5}} dot={generateDot}  />
       </ComposedChart>
    </ResponsiveContainer>
);
